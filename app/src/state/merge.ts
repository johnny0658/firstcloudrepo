 ...
 * on top — same symbol sums shares (shorts net against longs), cash adds.
 * Negative shares are short positions; rows that net to ~zero are dropped.
 */
export function mergePortfolio(
  ...
  return {
    holdings: [...bySymbol.entries()]
      .filter(([, shares]) => Math.abs(shares) > 1e-9)
